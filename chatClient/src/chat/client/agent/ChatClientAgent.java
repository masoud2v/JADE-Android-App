 ...
 * ChatManagerAgent. 
 * - ChatListener. A CyclicBehaviour that handles messages
 * from other chat participants. 
 * - ChatSpeaker. A OneShotBehaviour that sends a
...
public class ChatClientAgent extends Agent implements ChatClientInterface {
	...
	private void notifyParticipantsChanged() {
		...
		logger.log(Level.INFO, "Participants Changed Sending broadcast " + broadcast.getAction());
		context.sendBroadcast(broadcast);
	}
	...
	private void notifySpoken(String speaker, String sentence) {
		...
		logger.log(Level.INFO, "Notify Spoken Sending broadcast " + broadcast.getAction());
		context.sendBroadcast(broadcast);
	}
	/*
	 * NotifyActivity updates this ChatClientAgent's ChatTextField with message saying that newParticipant is added.
	 */
	private void notifyActivity(String sentence) {
		Intent broadcast = new Intent();
		broadcast.setAction("jade.demo.chat.REFRESH_CHAT");
	...
	class ParticipantsManager extends CyclicBehaviour {
		...
		public void onStart() {
			...
			subscription.setReplyWith(replyWith);	//TASK1 Change
		 ...
		 * Task 1: All the participants send subscription message by setting Reply-With as their localNames
		 * When ChatManager notifies all subscribers with createReply.clone() message created from subscription message (ACLMessage.SUBSCRIBE) from newParticipant
		 * ReplyWith is changed to respective subscriber's AIDs
		 * Comparing Reply-With (Sent value) with In-Reply-To we can identify newParticipant
		 * Only for newParticipant these attributes remain same, for others Reply-With will be newParticipant and In-Reply-To will be their own localName
		 */
				...
				if (msg.getPerformative() == ACLMessage.INFORM) {
					try {
						...
						if(p instanceof Joined) {
							...
							String sentence;
							//Changes for Task1 STARTS
							newUser = new AID(msg.getReplyWith(),true).getLocalName().equals(msg.getInReplyTo());
							if(newUser){
								for(AID a : aid) {
									participants.add(a);
								}
								...
								for(AID a : aid) {
									sentence = a.getLocalName() + " has entered the chat room";
									notifyActivity(sentence);
									participants.add(a);
								}
								notifyParticipantsChanged();
							}				
							//Changes for Task1 ENDS
						}
						if(p instanceof Left) {
							...
							for(AID a : aid){
								String sentence = a.getLocalName() + " has just Left the chat room!";
								notifyActivity(sentence);
	...
	private class ChatSpeaker extends OneShotBehaviour {
		...
		public void action() {
			...
			while (it.hasNext()) {
				AID thisAID = (AID) it.next();
				spokenMsg.addReceiver(thisAID);
			}
	...
	private void handleUnexpected(ACLMessage msg) {
		if (logger.isLoggable(Level.WARNING)) {
			logger.log(Level.WARNING, "Unexpected message received from "
